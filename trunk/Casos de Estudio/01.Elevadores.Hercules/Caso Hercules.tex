...
\begin{Large}
 \begin{center}
		\underline{An\'{a}lisis Caso H\'{e}rcules} \linebreak \linebreak
        Grupo Nro:	R2
\end{center}
\end{Large}
}
...
La empresa tiene un problema de procesos ya que no sabe cuales agregan valor y cuales no. La misma se encuentra sobrepasada de cosas y no tiene una noci\'on v\'alida de que procesos son los que realmente le dan un valor. Esto resulta un grave problema ya que le trae como consecuencia otros problemas que ponen en jaque la situaci\'on de la misma.
\paragraph{Soluci\'on}
Se debe determinar los procesos que agregan valor y los que no y a partir de alli tomar decisiones sobre si realmente valen la pena aquellos procesos que no agreguen valor, es decir si son realmente \'utiles o fundamentales para la empresa ya que los mismos no generan ni un valor y pueden traer otros problemas. Por eso es necesario un replanteamiento sobre todos los procesos de la misma y decisiones concretas.

\paragraph{Situaci\'on}
La empresa tiene un grave problema de costos. Se encuentra desbordada de pedidos y no tiene una respuesta eficiente. Para poder intentar hacerle frente a los mismos toma decisiones equivocadas y apuradas, m\'as que nada. Intentar vender m\'as en este caso no le esta garantizando ganar m\'as ya que en alg\'un momento esta situaci\'on de toma de decisiones erroneas va a hacer que el rendimiento comience a bajar notoriamente. Las principales decisiones erroneas son:
	\begin{itemize}
 		\item Coloca personal incapacitado para ciertos puestos. 
		\item No tiene un control de lo que produce, puede estar produciendo cosas equivocadas o m\'as de lo que realmente necesita ya que no cuenta con un plan eficiente.
		\item Acepta cualquier pedido sin saber si realmente llega a cumplimentarlo. Hace todo a medida del cliente lo cual no favorece a la producci\'on ya que la misma no esta estandarizada lo cual hace que aumente el margen de error.
	\end{itemize}
\paragraph{Soluci\'on}
Para intentar solucionar el problema de costos la empresa deber\'ia estandarizar los procesos de producci\'on. Elegir una serie de modelos, armar un catalogo para mostrar a los clientes y fabricar una serie de tipos de elevadores. Con esto se puede tener una mejor producci\'on, bajando el margen de error y teniendo un mejor control sobre lo producido. Con esto podr\'ia llegar a perder una parte del mercado, pero consideramos que no ser\'{a} una gran parte y esto lo beneficiar\'{a} ya que ofrecer\'{a} un mejor servicio. No fabrica  descontroladamente, no fabrica de m\'as, puede contar con un stock reutilizable y los costos se reducen notablemente.
Proponemos que el proceso de fabricaci\'on de piezas sea tercerizado, con esto bajar\'a la dificultad del planeamiento, se tendr\'{a} un menor sector de producci\'on m\'{a}s controlable, m\'as eficiente. Esto har\'a que los costos se puedan reducir.
Para todo esto se debe contratar personal eficiente y que sepa de la tarea que va a realizar, por lo menos un director o gerente de planeamiento de producci\'on que tenga conocimiento del tema y t\'ecnicos especializados en el \'area.
